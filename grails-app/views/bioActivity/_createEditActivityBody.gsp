...
<span class="container-fluid validationEngineContainer" id="validation-container">
...
<r:script>
        ...
        var Master = function () {
             ...
             */
            this.save = function () {
                $("#validation-container").bind("jqv.form.validating", function() {
                  $('#erroronuploadingphoto').empty();

                });
                $("#validation-container").bind("jqv.field.result", function(event,field, errorFound,prompText) {
                  if(errorFound){
                      if ($(field).attr('id')=='uploadingphoto'){
                          // promptText contain </br>
                          //$('#erroronuploadingphoto').addClass('label').addClass('label-danger').prop('style', 'background-color: #EE0100').text(prompText)
                          $('#erroronuploadingphoto').addClass('label').addClass('label-danger').prop('style', 'background-color: #EE0100').text('* You must fill one of the following fields')
                          //Tried to attach the error div generated by validating, but show control is not very well
                          //$('#erroronuploadingphoto').append($(field).siblings('.formError'))
                          }
                  }
